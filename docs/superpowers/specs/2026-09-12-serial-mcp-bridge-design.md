...
**实现含义（关键）**：`requestPort()` 每次调用都会弹出选择框并要求用户手势——**即使已存在授权端口也一样**。因此自动重连**绝不能**走 `requestPort()`，必须直接取 `getPorts()` 返回的端口对象调 `open()`。这条策略落在 `serialProvider.requestPort()` 的实现里——它优先返回已授权端口，仅在没有时才回退到 `navigator.serial.requestPort()`（见第 5.2 节）。漏掉这一步，"授权一次后全自动"的承诺就不成立。
...
`id` 由 `mcp-server.js` 生成（适配器内唯一）。桥按"适配器身份 + id"路由响应。
